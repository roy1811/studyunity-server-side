...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@studyunity.qqdmwrr.mongodb.net/`;
const client = new MongoClient(uri, { useNewUrlParser: true, useUnifiedTopology: true, serverApi: ServerApiVersion.v1 });

async function run() {
  try {
    await client.connect();
    const db = client.db('studyunity');
...
app.get('/', (req, res) => {
  res.send('Hello Studyunity server')
})


// running server
app.listen(process.env.PORT || 7000, () => {
  console.log('studyunity server running', port)
})